...
void MainThread_Entry(ULONG thread_input)
{
  ...
  status = txm_module_object_pointer_get(TXM_QUEUE_OBJECT, "Resident Rx Queue", (VOID **)&ResidentRxQueue);
  if(status)
  {
    Error_Handler();
  }

  status = txm_module_object_pointer_get(TXM_QUEUE_OBJECT, "Resident Tx Queue", (VOID **)&ResidentTxQueue);
  if(status)
  {
    Error_Handler();
  }  
  ...
  tx_queue_receive(ResidentTxQueue, &rx_msg, TX_WAIT_FOREVER);
